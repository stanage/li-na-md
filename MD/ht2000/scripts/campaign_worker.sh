...
# index/run_manifest.csv where (row_index mod n_lanes) == lane, so the 2000
# runs spread evenly over every lane on every node with no coordination and
# no shared work queue to corrupt.
#
