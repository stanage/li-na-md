...
This is the answer to "where do the .itp/.gro files come from?" -- they are
pulled, one SMILES at a time, from the Yale LigParGen web service.

For ht1000 most of the work is already done: import_solvent_ff.py copies the
272 solvents ht2000 had already fetched and verified, so this script normally
only handles the handful that are new. Point it at --smiles-file to fetch an
explicit list.
